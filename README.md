...
2. 控制手机用 **OTG 线** 连接被控手机
3. 在本 App 点「OTG 首次激活」，被控端弹出授权时点允许
4. 成功后拔掉 OTG，两机连同一 Wi‑Fi / 热点，用填入的 IP:5555 投屏

也可直接点「OTG 投屏」在 USB 上镜像（无需无线；线不要拔）。
...
也可在开发者选项中开启「无线调试」，使用其中显示的 **IP:端口**（不是配对端口）。
